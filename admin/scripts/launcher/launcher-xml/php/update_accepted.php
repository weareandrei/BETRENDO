...
foreach ($shops as $shop) {
    $sql = "
    UPDATE catalog_".$shop."_accepted
       SET catalog_".$shop."_accepted.Price = (SELECT catalog_".$shop.".Price
  									       FROM catalog_".$shop."
									      WHERE catalog_".$shop.".XML_Reference_ID=catalog_".$shop."_accepted.XML_Reference_ID
                                        ),
           catalog_".$shop."_accepted.Oldprice = (SELECT catalog_".$shop.".Oldprice
  									            FROM catalog_".$shop."
									           WHERE catalog_".$shop.".XML_Reference_ID=catalog_".$shop."_accepted.XML_Reference_ID
                                            );
    ";
    ...
    if (!$data_update) {
        $json_return_data = array(
            "ReturnCode" => -1, // Error
            "ReturnMessage" => ("Ошибка обновления старых данных SQL:" . $sql),
        );
