...
function load_products_from_DB(by_filter,sort) {
    ...
    xhr.onload = function() {
        ...
        //console.log(json_returned_products['time']);
    ...
    if ($("#directory-sub-category").length) {
        ...
        PRODUCT_REQUEST_JSON.sub_categories.push($('#directory-sub-category').data("name"));
    } else {
        ...
        $('.tag').each(function() {
            ...
            if (filter_type == 'categories') {
                PRODUCT_REQUEST_JSON.sub_categories.push($(this).data("cat-id"));
            } 
        });
    }
    ...
    if (by_filter == true) {
        ...
        $('.product-item').each(function() {
            // Удаляем все товары чтобы заново их загрузить
            $(this).remove();
        });
    ...
    console.log(PRODUCT_REQUEST_JSON);
